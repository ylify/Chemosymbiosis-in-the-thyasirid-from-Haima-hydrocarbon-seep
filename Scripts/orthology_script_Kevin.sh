...
MIN_TAXA=19 #Minimum number of OTUs to keep an OG
HALF=19 #50% the number of final OTUs
...
#cd ..
#mv phylopypruner_output phylopypruner_output-min_4_taxa

phylopypruner --threads $CORES --dir . --min-taxa $HALF --min-len $MIN_SEQUENCE_LENGTH --min-support 0.75 --mask pdist --trim-divergent 0.75 --min-pdist 0.01 --prune MI #Run with version 0.9.5
cd phylopypruner_output
FastTreeMP -slow -gamma supermatrix.fas > FastTree.tre

mv partition_data.txt 'iqtree_min_'$MIN_TAXA'_'$HALF'_partiton'.txt

iqtree2 -T AUTO -B 1000 -s supermatrix.fas -Q 'iqtree_min_'$MIN_TAXA'_'$HALF'_partiton'.txt -m MFP
cd ..
mv phylopypruner_output 'phylopypruner_output-'$MIN_TAXA'_'$HALF'half_taxa'

#phylopypruner --threads $CORES --dir . --min-taxa 26 --min-len $MIN_SEQUENCE_LENGTH --min-support 0.75 --mask pdist --trim-divergent 0.75 --min-pdist 0.01 --prune MI --include Achatina_fulica Mizuhopecten_yessoensis#Run with version 0.9.5 
#cd phylopypruner_output
#FastTreeMP -slow -gamma supermatrix.fas > FastTree.tre
#iqtree2 -T AUTO -B 1000 -s supermatrix.fas -Q partition_data.txt -m MFP
#cd ..
#mv phylopypruner_output phylopypruner_output-min_26_taxa
